...
coef(covFitCV)[1:d] <- param_LOO$thetaOpt
coef(covFitCV)[d+1] <- param_LOO$sigmaOpt2
...
out = metrop(logLikFun0, initial = fitMLE$covariance@par, nbatch = 1e5,  scale = 0.76)
...
alpha = 0.05
result <- optimize(wasserstein_shift, interval = c(0.1,5))
